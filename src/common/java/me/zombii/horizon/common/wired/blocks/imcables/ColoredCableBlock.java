...
public class ColoredCableBlock implements IEnergyBlock {
    ...
    public static final Identifier GENERAL_ID = Identifier.of(HorizonCommon.NAMESPACE, "colored-cable");
...
//        this.modelGenerator.getGroup("base").cuboids.add(collisionCube);
    ...
    public boolean canConnect(BlockState state, Direction direction, BlockState target) {
        ...
        if (modBlock instanceof ColoredCableBlock) {
            return this.getColor().equals(((ColoredCableBlock) modBlock).getColor());
        }

        IGameTagList list = target.getTags();
        return list.contains(HorizonTags.TAG_CABLE_CONNECTABLE);
    }
