package me.zombii.horizon.common.wired.connectedblocks;
...
public class CableConnectorFunctionBe implements ISidedBlockConnector.ConnectorFunction {

    EnhancedBlockModelGenerator childModelGenerator;
    public final String defaultModelName;

    final Identifier id;
    final IModBlock networkedBlock;

    public CableConnectorFunctionBe(
            IModBlock networkedBlock,
            EnhancedBlockModelGenerator modelGenerator,
            Identifier id
    ) {
        this.id = id;
        ...
        this.childModelGenerator = modelGenerator;
        this.childModelGenerator.setName(defaultModelName);
    ...
    public boolean canConnect(
        ...
        boolean canConnectA = wireBlockEntity.canConnect(state, target, targetBe, direction);
        if (!canConnectA) return false;

        if (targetBe instanceof IEnergyBE energyBE) {
            return energyBE.canConnect(target, state, wireBlockEntity, direction.getOpposite());
        }
        return false;
    }
...
//        int powerLevel = blockState.getIntParam("powerLevel", 0);
        ...
        WireBE self = (WireBE) zone.getBlockEntity(x, y, z);
        BlockEntity PX_BE = zone.getBlockEntity(x + 1, y, z);
        BlockEntity NX_BE = zone.getBlockEntity(x - 1, y, z);
        BlockEntity PY_BE = zone.getBlockEntity(x, y + 1, z);
        BlockEntity NY_BE = zone.getBlockEntity(x, y - 1, z);
        BlockEntity PZ_BE = zone.getBlockEntity(x, y, z + 1);
        BlockEntity NZ_BE = zone.getBlockEntity(x, y, z - 1);

        boolean PX_ON = PX != null && canConnect(self, blockState, Direction.POS_X, PX, PX_BE);
        boolean NX_ON = NX != null && canConnect(self, blockState, Direction.NEG_X, NX, NX_BE);
        boolean PY_ON = PY != null && canConnect(self, blockState, Direction.POS_Y, PY, PY_BE);
        boolean NY_ON = NY != null && canConnect(self, blockState, Direction.NEG_Y, NY, NY_BE);
        boolean PZ_ON = PZ != null && canConnect(self, blockState, Direction.POS_Z, PZ, PZ_BE);
        boolean NZ_ON = NZ != null && canConnect(self, blockState, Direction.NEG_Z, NZ, NZ_BE);
...
//        Arrays.fill(pipeBlockLightLevels, (short) (powerLevel != 0 ? 63: 0));
        ...
        pipeBlockLightLevels = blockLightLevels;
